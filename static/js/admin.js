...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function configurarEventosBackup() {
        ...
        const btnStatusBackup = document.getElementById('btnStatusBackup');
        const btnListarBackups = document.getElementById('btnListarBackups');
        const btnCriarBackupModal = document.getElementById('btnCriarBackupModal');
        const btnAtualizarLista = document.getElementById('btnAtualizarLista');
        
        if (btnCriarBackup) {
            btnCriarBackup.addEventListener('click', async (e) => {
                e.preventDefault();
                await criarBackup();
            });
        }
        ...
        if (btnStatusBackup) {
            btnStatusBackup.addEventListener('click', async (e) => {
                e.preventDefault();
                await carregarStatusBackup();
            });
        }
        ...
        if (btnListarBackups) {
            btnListarBackups.addEventListener('click', async (e) => {
                e.preventDefault();
                abrirModalBackups();
            });
        }
        
        if (btnCriarBackupModal) {
            btnCriarBackupModal.addEventListener('click', async (e) => {
                e.preventDefault();
                await criarBackup();
            });
        }
        ...
        if (btnAtualizarLista) {
            btnAtualizarLista.addEventListener('click', async (e) => {
                e.preventDefault();
    ...
    async function criarBackup() {
        try {
            ...
            const btnCriarModal = document.getElementById('btnCriarBackupModal');
            
            // Desabilitar botões durante o processo
            if (btnCriar) btnCriar.disabled = true;
            if (btnCriarModal) btnCriarModal.disabled = true;
            ...
            if (data.success) {
                mostrarAlerta(`Backup criado com sucesso: ${data.backup.name}`, 'success');
                // Atualizar status se o modal estiver aberto
                if (document.getElementById('modalBackups').classList.contains('show')) {
                    await carregarStatusBackup();
                    await carregarListaBackups();
                }
            } else {
                mostrarAlerta(`Erro ao criar backup: ${data.message}`, 'danger');
            }
        } catch (error) {
            console.error('Erro ao criar backup:', error);
            mostrarAlerta('Erro ao criar backup', 'danger');
        } finally {
            // Reabilitar botões
            if (btnCriar) btnCriar.disabled = false;
            if (btnCriarModal) btnCriarModal.disabled = false;
        }
    }

    // Carregar status dos backups
    async function carregarStatusBackup() {
        try {
            ...
            if (data.success) {
                ...
                const statusElement = document.getElementById('statusBackup');
                const statusDetalhadoElement = document.getElementById('statusBackupDetalhado');
                ...
                if (statusElement) {
                    statusElement.innerHTML = `
                        ...
                        <div class="small">
                            ...
                            Último: ${status.last_backup ? new Date(status.last_backup.timestamp).toLocaleString('pt-BR') : 'Nenhum'}
                        </div>
                    `;
                }
                
                if (statusDetalhadoElement) {
                    statusDetalhadoElement.innerHTML = `
                        <div class="row">
                            <div class="col-6">
                                <div class="text-center">
                                    <h4 class="text-primary">${status.total_backups}</h4>
                                    <small>Total de Backups</small>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="text-center">
                                    <h4 class="text-success">${status.completed_backups}</h4>
                        ...
                        <div class="row mt-2">
                            <div class="col-6">
                                <div class="text-center">
                                    <h4 class="text-danger">${status.failed_backups}</h4>
                                    <small>Falharam</small>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="text-center">
                                    <h4 class="text-info">${status.total_size_mb} MB</h4>
                                    <small>Espaço Total</small>
                                </div>
                            </div>
                        </div>
                        <hr>
                        <div class="small">
                            ...
                            ${status.last_backup ? 
                                `${status.last_backup.name}<br>
                                 <span class="text-muted">${new Date(status.last_backup.timestamp).toLocaleString('pt-BR')}</span>` : 
                ...
                mostrarAlerta(`Erro ao carregar status: ${data.message}`, 'danger');
            }
        } catch (error) {
            console.error('Erro ao carregar status dos backups:', error);
            mostrarAlerta('Erro ao carregar status dos backups', 'danger');
        }
    }
    ...
    async function carregarListaBackups() {
        try {
            ...
            if (data.success) {
                ...
                if (tbody) {
                    ...
                    tbody.innerHTML = data.backups.map(backup => `
                        <tr>
                            ...
                            <td>${backup.name}</td>
                            <td>
                                <span class="badge bg-${backup.type === 'manual' ? 'primary' : 
                                                   backup.type === 'automated' ? 'success' : 
                                                   backup.type === 'weekly' ? 'info' : 'secondary'}">
                                    ${backup.type}
                                </span>
                            </td>
                ...
                mostrarAlerta(`Erro ao carregar lista de backups: ${data.message}`, 'danger');
            }
        } catch (error) {
            console.error('Erro ao carregar lista de backups:', error);
            mostrarAlerta('Erro ao carregar lista de backups', 'danger');
        }
    }
    ...
    async function downloadBackup(backupId) {
        try {
            ...
            mostrarAlerta('Erro ao baixar backup', 'danger');
        }
    }

    // Abrir modal de backups
    function abrirModalBackups() {
        const modal = new bootstrap.Modal(document.getElementById('modalBackups'));
        modal.show();
